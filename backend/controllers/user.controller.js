...
export const updateUser = async (req, res) => {
    try {
        ...
        if (username && username !== user.username) {
            const usernameAlreadyPresent = await User.findOne({ username });
        ...
        if (profileImg) {
            if (user.profileImg) {
                //if profileImg already exist we need to delete it from cloudinary to save space
                await cloudinary.uploader.destroy(
                    user.profileImg.split("/").pop().split(".")[0],
                ); //cutting the last part of the imgUrl and passing it as argument
            }
        ...
        if (coverImg) {
            if (user.coverImg) {
                //if coverImg already exist we need to delete it from cloudinary to save space
                await cloudinary.uploader.destroy(
        ...
        user.password = null; //removing password before sending updated info as response
